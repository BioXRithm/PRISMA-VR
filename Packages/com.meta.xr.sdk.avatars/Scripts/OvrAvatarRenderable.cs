...
#pragma warning restore CA2213 // Disposable fields should be disposed
        ...
        protected void CopyMaterial()
        {
            if (_materialCopy == null)
            {
                ...
                {
                    _materialCopy = new Material(EmergencyFallbackShader);
                }
                else
                {
                    _materialCopy = new Material(sharedMaterial);
                    // NOTE: when the material is copied here, extra care should be taken to copy uniform arrays
                    OvrAvatarSubMeshIdentification.DeepCopyFloatArrays(_materialCopy, sharedMaterial);
                }
        ...
        protected internal virtual void ApplyMeshPrimitive(OvrAvatarPrimitive primitive)
        {
            ...
            rendererComponent.sharedMaterial = primitive.material;  // NOTE: after this copy, make sure to deep copy all array types
            ...
            // copy all sub-mesh identifiers and material parameter structures into the material's scope
            OvrAvatarSubMeshIdentification.DeepCopyFloatArrays(rendererComponent.sharedMaterial, primitive.material);
        ...
        private static Shader _celShader = null;
        public static Shader CelShader
        {
            get
            {
                if (_celShader == null)
                {
                    ...
                    var configuration = shaderManager.GetConfiguration(ShaderType.Cel);
                    if (configuration == null) { return null; }

                    _celShader = configuration.Shader;
                }
                return _celShader;
            }
        }
