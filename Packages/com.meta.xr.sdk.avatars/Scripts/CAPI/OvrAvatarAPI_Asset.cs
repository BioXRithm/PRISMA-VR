...
namespace Oculus.Avatar2
{
    public static partial class CAPI
    {
        ...
        public static bool OvrAvatar2Primitive_GetMaterialBuffers(
            ovrAvatar2Id id,
            in NativeArray<float> types, // float*, must be of size subMeshCount * slotsPerSubMesh
            in NativeArray<float> offsets, // float*, must be of size subMeshCount * slotsPerSubMesh
            in NativeArray<float> parameterBuffer, // float*
            UInt32 slotsPerSubMesh,
            out UInt32 count,
            UInt32 maxCount,
            ...
            UnityEngine.Object? context = null)
        {
            unsafe
            {
                return ovrAvatar2Primitive_Prototype_GetMaterialBuffers(id, types.GetPtr(), offsets.GetPtr(),
                parameterBuffer.GetPtr(), slotsPerSubMesh, out count, maxCount, out bufferSize, parameterBuffer.GetBufferSize())
                .EnsureSuccess("ovrAvatar2Primitive_GetMaterialBuffers", assetLogScope, context);
            }
        }

        // Get all parameter data in structs that can be transferred directly to the renderer
        [DllImport(LibFile, CallingConvention = CallingConvention.Cdecl)]
        private static unsafe extern CAPI.ovrAvatar2Result ovrAvatar2Primitive_Prototype_GetMaterialBuffers(
            ovrAvatar2Id id,
            float* types,       // must be of size subMeshCount * slotsPerSubMesh
            float* offsets,     // must be of size subMeshCount * slotsPerSubMesh
            float* parameterBuffer,
            UInt32 slotsPerSubMesh,
            out UInt32 subMeshCount,
            UInt32 maxSubMeshCount,
            out UInt32 parameterBufferSize,
            UInt32 maxParameterBufferSize);
